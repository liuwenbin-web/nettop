...
<title>dubbo服务拓扑结构_application</title>
...
<script>
	...
	function draw(json) {
		...
		var providerKey = "";
		var consumerKey = "";
		var preProviderNode = null;
		var preConsumerNode = null;
		var endConsumerNode = "";
		var endProviderNode = "";
		var h = false;
		var toArr = {};
		...
		$.each(json,function(i, n) {
			...
			$.each(providers, function(p_i, p_n) {
				//如果不存在，画节点
				providerKey = providerHead + p_n.application;
				if (undefined == map[providerKey]) {
					console.log(providerKey);
					//额为再加5个汉字的大小
					providerNodeW = p_n.interfaceName.length * 7.5+12*5;
					var text = texts(["应用名称："+p_n.application,"服务IP："+p_n.ip],providerNodeW,true);
					var afterText = texts(["应用名称："+p_n.application,"接口提供者个数："+n.providersCount,"服务IP："+p_n.ip,"服务端口："+p_n.port,"协议名称："+p_n.protocol,"开发者："+p_n.owner],providerNodeW,true);
			...
			$.each(consumers, function(c_i, c_n) {
				//如果不存在，画节点
				consumerKey = consumerHead + c_n.application;
				if (undefined == map[consumerKey]) {
					console.log(consumerKey);
					//额为再加5个汉字的大小
					consumerNodeW = c_n.interfaceName.length * 7.5+12*5;
					var text = texts(["应用名称："+c_n.application,"服务IP："+c_n.ip],consumerNodeW,false);
					var afterText = texts(["应用名称："+c_n.application,"服务IP："+c_n.ip,"接口消费者个数："+n.consumersCount],consumerNodeW,false);
			...
			$.each(fromArr,function(fromService,fromNode){
				$.each(toArr,function(toService,toNode){
					if(linkMap[fromService+"-->"+toService] == undefined){
						...
						console.log(fromService+"-->"+toService);
					}
				});
			});
			fromArr={};
			toArr={};
			/*if(linkMap[consumerKey+"-->"+providerKey] == undefined){
				linkNode(map[consumerKey], map[providerKey], 2,true);
				linkMap[consumerKey+"-->"+providerKey]="";
			}*/
		...
		var maxContainerRight = maxEndConsumerNodeRight;
